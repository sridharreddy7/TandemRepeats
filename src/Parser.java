...
public class Parser {
	public static void main(String args[]){
		...
		int scoreThreshold = Integer.parseInt(args[1]);
		TandemDP dp = new TandemDP();
		TandemSASI sa = new TandemSASI();
		for(String key: database.keySet()){
			String sequenceA = database.get(key);
			dp.start(sequenceA, scoreThreshold);
			sa.start(sequenceA, scoreThreshold);
		}	
	}
