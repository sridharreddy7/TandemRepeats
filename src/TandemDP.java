...
//		if (shaded[row][0] != -1 && col >= shaded[row][0] && col <= shaded[row][1]) { // if
//			// the
//			// column
//			// is
//			// in
...
//			if (pathpos.getRow() == row && pathpos.getCol() == col) { // if the
//				// position
//				// matches
//				// the
//				// given
...
//					return true; // position matches row and col and prevrow,
//				// prevcol
//
//				else
//					return false; // position matches row and col but not
//				// prevrow and prevcol
//			}
//		}
...
//
//	/*
//	 * isPath takes an array of Position that are on the path, and two ints
//	 * which represent a row and column isPath returns true if the path contains
//	 * the Position that is represented by the row and col
//	 */
//	public static boolean isPath(Position[] path, int row, int col) {
//		Position pathpos = path[0]; // assign the value of the first position on
//		// the path
//		// iterate through the path array and look for a position that matches
//		// the given row and col
//		for (int i = 0; i < path.length; i++) {
//			pathpos = path[i]; // assign a new value to pathpos
//			if (pathpos.getRow() == row && pathpos.getCol() == col)
//				return true; // if the row and col of that pathpos is equal to
//			// the passed row and col, return true
//		}
//		return false; // the path does not have a Position with a row and col
//		// equal to the passed row and col
//	}
//
...
//				else
//					score = MISMATCH;
//				max = Math.max(Math.max(matrix[i - 1][j - 1] + score, matrix[i - 1][j] + GAP_PENALTY),
//						Math.max(matrix[i][j - 1] + GAP_PENALTY, 0));
//				// check if this cell comes from shaded cell
//				boolean comesFromShaded = !((max == matrix[i - 1][j - 1] + score) && !isShaded(shaded, i - 1, j - 1) || // diag
//						// score
//						// is
//						// max,
//						// and
...
//						// not
//						// shaded
//						(max == matrix[i - 1][j] + GAP_PENALTY) && !isShaded(shaded, i - 1, j) || // top
//						// score
//						// is
//						// max,
//						// and
...
//						// not
//						// shaded
//						(max == matrix[i][j - 1] + GAP_PENALTY) && !isShaded(shaded, i, j - 1)); // left
//				// score
//				// is
//				// max,
//				// and
...
//				// shaded
//
//				if (max != 0 && comesFromShaded) // if the nonzero max comes
//					// from shaded
//				{
//					if (shaded[i][0] == -1) // if the leftmost shaded column
//						// hasn't been found yet
//						shaded[i][0] = shaded[i][1] = j; // set leftmost &
//					// rightmost
//					else if (shaded[i][0] != -1 && j < shaded[i][0]) // if
//						// current
//						// column
//						// is
//						// less
...
//						shaded[i][0] = j;
//					// set leftmost shaded
//					else if (j > shaded[i][1]) // if the current column is
//						// greater than the rightmost
//						shaded[i][1] = j; // update rightmost shaded column
//				}
//
//				if (shaded[i][0] != -1
//						&& !(isShaded(shaded, i - 1, j) || isShaded(shaded, i - 1, j + 1) || isShaded(shaded, i, j)))
//					foundEnd = true;
//
